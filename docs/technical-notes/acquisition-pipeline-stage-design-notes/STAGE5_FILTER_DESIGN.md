...
| `lf_explicit_minutes` | `build_signals.instructional_declaration()` — a NUMBER + an instruction referent + DAY scope, minus an interval/threshold antecedent (**#683**, 2026-07-29; the bare `INSTRUCTIONAL_RE` is NOT the predicate) | **+target (strong)** | the "golden nugget" path. Was the bank's WORST target detector (acc 0.4545) because "first 30 minutes of class" / "practice 10 minutes per day" read as declarations; now **acc 1.0**, narrow-but-trustworthy (`cov` 0.0063→0.0006). Missed declarations are a VISION problem, not a looser regex — REQ-093's standing lesson |
...
| `lf_staff_day` | the STAFF day (**#684**, 2026-07-29): employment-obligation CLAUSES (a staff subject → a duty verb → the time: *"Elementary staff … are to report to work by 7:15 a.m. and remain until 3:00 p.m."*) govern MORE of a text basis's in-window times than student-referent language does — `build_signals.staff_duty_positional` per basis, `detectors.staff_day_owned` as the ONE predicate | **−negative (strong)**, `UNDERMINE_CLASS` **hard** | an employee handbook's report-time table has every shape a bell schedule has (real table, in-window pairs, the literal words `bell schedule`/`school start time`) — the shape is right, the **referent** is wrong. **Hard, not soft:** a soft wrong-*day* mention leaves a real table sending (#60/#528) because the table is still the student day; here the table *is* the staff day. Also consulted by **every** `STRONG_STRUCTURAL` detector (`lf_heading_hours`, `lf_footer_hours`, `lf_explicit_minutes` — the #705 review found guarding only the heading left the footer path re-opening the auto-send; a test pins the closure) — a negative vote alone could not fix #684, since nothing undermines `STRONG_STRUCTURAL` |
...
nearest heading/segment reads `office`/`staff`/`workday`, or — since #684 — a heading/footer block
sitting over staff-owned duty times) points to building/office hours rather than the school day — the
