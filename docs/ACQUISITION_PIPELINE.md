...
### 1 · Queue — built 2026-06-22 · deep design + decision log: `docs/technical-notes/STAGE1_QUEUE_DESIGN_2026-06.md`

**Purpose:** build a batch of districts + per-band school lists to target — the structured, `gate@1`-reviewed input Stage 2/3 consume. **In:** NCES `ccd_lea_029` / `ccd_sch_029` + DB enrollment/staffing/`is_shared_service_entity`. **Out:** `data/acquisition/queue/batch_NNNNN.json` (structured targeting only, no prompts; schema `batch.example.json`). Code: `stage1_queue/queue_batch.py` (+ `common/school_sampling.py`, `common/district_status.py`).
...
- **`gate@1` (was Checkpoint A):** human review that the right districts/schools/bands are targeted — out-of-band today; an in-band approval event once the console lands (governance §11).
