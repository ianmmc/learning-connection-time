...
- **Console (REQ-110, built 2026-06-28):** ungated health/emergent readout (per-district outcome/counts, the `err` failure breakdown, the CMS/host distribution — all read from the **DB cross-stage cache**) + a per-district Node-capture run trigger (`stage3_capture/headless.py` `run_batch`; the Node `district <ROOT> <DIR>` mode keeps a run batch-scoped). The cross-stage cache is now a **live working store** maintained by each stage's finish hook (`common/cache_ingest.py`), not only the Stage-5 ingest. Detail: `STAGE3_CAPTURE_DESIGN_2026-06.md` §3.
