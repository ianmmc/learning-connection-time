...
**Status:** Stage 1 (Queue) designed, built, validated, and CP-A-approved (`batch_00001.json` ready to advance). Stage 2 (Discovery) designed, built, and run live against all 12 districts in `batch_00001` (12/12 `found_all`); billing/auth failure handling hardened afterward. Stage 3 (Capture) fully designed this session — Drive/Docs/Sheets/Slides branch, modal dismissal, unconditional `page.pdf()`, emergent-candidate link-following — **not yet built**. Stages 4-9 design validated (extraction); per-school council rules still to build.
...
- `captures/` subdirectory, files named by `md5(url).slice(0,10)` plus extension — already exactly how `capture_discovery.mjs` works today, independently arrived at twice.
- `captures.json` — per-candidate record (url, hash, tools/source/found_on, ok, kind, final_url, files, err) — a **separate file from `candidates.json`**, not a mutation of it, so Discovery's output stays untouched.
...
| **Capture (Stage 3): designed 2026-06-23, not yet built** | `infrastructure/scraper/capture_discovery.mjs` (active, to be extended) |
| Modal dismissal + page.pdf() options to port in (verified pure-Playwright, no dead-architecture coupling) | `infrastructure/scraper/src/capturer.ts` |
| Drive Tier-1 export-URL logic to salvage (Playwright-preview/Gemini tiers dropped) | `infrastructure/scripts/enrich/google_drive_handler.py` |
