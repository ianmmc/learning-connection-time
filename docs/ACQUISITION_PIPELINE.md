...
request-more-evidence loop's **detect/persist/review** is built and validated; **execution** (an approved
request firing the 7→6/3/2/1 back-edge) is not. Stages 8–9 are designed, not built. Detail on each stage's
...
3. **Council membership + decision rule — template decided, membership open.** 2-voters-2-families → judge-on-disagreement is built and validated (REQ-056, REQ-117); which specific models fill each slot is still open, pending the Council Lab's clean-data re-benchmark (GitHub #80). See `STAGE6_DISPATCH_DESIGN_2026-06.md` §3A.
