# Daily Instructional Minutes: End-to-End Acquisition + Extraction Strategy

> **Status:** Strategy report / decision record — 2026-06-12; **validated & extended 2026-06-13.**
> **Purpose:** Weigh end-to-end options (cost vs. time vs. accuracy) for capturing daily instructional minutes per grade band for all ~20,000 U.S. districts, after the local-first pipeline hit an extraction-accuracy wall. Companion to `EXTRACTION_BENCHMARK_FINDINGS.md` (the benchmark this builds on) and `INSTRUCTIONAL_TIME_HARVEST.md` (why SEA central data is a dead end).
>
> **2026-06-13 update — the strategy below was tested and the core bets held.** Paid-API extraction is cheap and capable: **Gemini 2.5 Flash leads the full-41 at 68.9% and is among the cheapest**; on *good* inputs (difficulty > 0.70) top models hit ~95–100%. The binding constraint is **input quality, not model choice** (20% of districts solved by zero models; the hard inputs failed on granularity/noise, not OCR). **Discovery via domain-scoped search works** (Perplexity / OpenRouter `gpt-4o-mini-search` / Claude WebSearch; Google grounding can't site-restrict). Direction confirmed: cheap-cloud council + format-aware capture + **per-school targeting** + statutory fallback. Full detail: `docs/technical-notes/EXTRACTION_AND_DISCOVERY_LEARNINGS_2026-06.md`.
