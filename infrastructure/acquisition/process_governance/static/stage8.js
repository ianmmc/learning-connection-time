...
(function () {
  ...
  function renderBand(band, b) {
    ...
    return `<section class="s8-band">
      ...
      ${s.n_outside_pool > 0 ? `<p class="s8-note" data-feat="pool-gap">⚠ <strong>${s.n_outside_pool}</strong> of the ${s.n_total} schools in this denominator are outside this band's Stage-1 sampling pool — placed in another band by anti-dilution (a K-8 serving middle sits in elementary's pool) and/or dropped by the ≤12/band queue-time cap — so coverage here is structurally capped at ${s.n_total - s.n_outside_pool}/${s.n_total}: the pipeline never seeks them for this band. Their grades still inherit this band's modal minutes via the Stage-9 tie rule (#696).</p>` : ""}
      ${renderSlots(band, b)}
