...
def _b(per_district=None, per_run=None, rounds=None, per_district_total=None):
    return BUD.Budget(per_district_usd=per_district, per_district_total_usd=per_district_total,
                      per_run_usd=per_run, max_request_rounds=rounds)
...
def test_district_total_cap_is_cumulative_across_handoffs():
    # per-RUN spend is 0 (fresh handoff) but LIFETIME spend is near the total cap -> the total blocks
    g = BUD.BudgetGovernor(_b(per_district=1.0, per_district_total=3.0),
                           district_spent={"D1": 0.0}, district_total_spent={"D1": 2.9})
    assert g.district_would_exceed("D1", 0.2) is False        # per-run: 0.0 + 0.2 <= 1.0, fine
    assert g.district_total_would_exceed("D1", 0.2) is True    # total: 2.9 + 0.2 > 3.0, blocked
    assert g.district_total_would_exceed("D1", 0.05) is False  # total: 2.9 + 0.05 <= 3.0
    assert g.district_total_would_exceed("D2", 0.1) is False   # a fresh district


def test_record_increments_both_per_run_and_total():
    g = BUD.BudgetGovernor(_b(), district_total_spent={"D1": 2.0})
    g.record("D1", 0.5)
    assert round(g.district_spent["D1"], 4) == 0.5       # per-run seed was empty -> 0.5
    assert round(g.district_total_spent["D1"], 4) == 2.5  # lifetime seed 2.0 -> 2.5
...
def test_none_caps_never_block():
    ...
    assert g.district_total_would_exceed("D1", 999.0) is False
...
def test_load_budget_reads_the_knob():
    ...
    assert b.per_district_total_usd is not None and b.per_district_total_usd >= b.per_district_usd
