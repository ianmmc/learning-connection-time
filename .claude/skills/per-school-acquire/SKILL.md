...
1. **Roster + queries.** Run:
   `python3 infrastructure/acquisition/discovery/per_school_run.py roster <ids> --max-schools 12`
   Writes `data/acquisition/per_school/<id>/roster.json` (per-school queries + `allowed_domains`).

2. **Wave 1 — Claude WebSearch (THIS IS THE AGENT'S JOB).** For each district, read its `roster.json` and spawn **one Haiku subagent per district** (chunk if a district has many schools). Instruct the subagent: use ONLY the `WebSearch` and `Write` tools; for each school run WebSearch on its `query` with `allowed_domains` set to the district domain (unscoped if empty); collect result URLs; **Write** `data/acquisition/per_school/<id>/claude_urls.json` as a JSON object `{"<school name>": ["url", ...], ...}` keyed EXACTLY by the `school` strings in roster.json. Do not guess URLs — only report what WebSearch returns. (Run subagents in background; they're independent across districts.)
...
3. **Wave 2 — OpenRouter residual.** After Wave 1 subagents finish, run:
   `python3 infrastructure/acquisition/discovery/per_school_run.py wave2 <ids> --cap 3`
...
4. **Flatten + dedup.** Run:
   `python3 infrastructure/acquisition/discovery/per_school_run.py flatten <ids>`
...
5. **Capture (tiered, local Playwright).** From `infrastructure/scraper/`:
   `node capture_discovery.mjs <abs path to data/acquisition/per_school> 6`
...
6. **Extract (Path-1 council) + aggregate + score.** Run:
   `python3 infrastructure/acquisition/discovery/council_extract.py <id>`
   Voters = Mistral Small 24B + Gemini 2.5 Flash-Lite + Qwen3-235B (3 families); judge = DeepSeek V3.2 (4th family) on no-consensus.
   **INVARIANT — extractors read TIMES; code computes MINUTES + MODE.** Each model returns only per-school `{start_time, end_time, grade_level, school_name}` facts it read. It NEVER computes minutes, subtracts anything, or picks a "typical"/modal schedule. `aggregate.py` does `gross = end − start` and the per-band mode in Python.
   **Target = GROSS daily minutes (end − start); no lunch/passing/recess, no assumed deductions.** **Consensus is on the per-school (start_time, end_time) pair** (per school, ±15 min, cross-family) — not on computed minutes. Cross-family agreement on the pair → accept the school's times; else judge (re-reads page; plausibility gate ~240–510 min). Then code: gross per school → **exact most-common mode** per band (genuine tie → mean). Score vs GT if present. Writes `council_result.json`.
